...
describe('<PhotoCard /> with default props', () => {
    ...
    it('should insert the prop values to correct places ', () => {
        ...
        expect(photoCardComp.find('.user-name').last().render().text()).toEqual(props.user);
        expect(photoCardComp.find('.carousel-image').render().attr('src')).toEqual(props.previewURL);
    });
});
